...
#ifndef _JOYLINK_AUTH_UECC_H_
#define _JOYLINK_AUTH_UECC_H_
...
uECC_Curve uECC_secp160r_1(void);
#endif
...
*/
typedef int (*uECC_RNG_Function_sap)(uint8_t *dest, unsigned size);
...
rng generate
*/
int	uECC_generate_rng_sap(uint8_t *buf,unsigned size);
...
*/
int uECC_make_key_sap(uint8_t *public_key, uint8_t *private_key, uECC_Curve curve);
...
int uECC_curve(void);
int uECC_verify_sap(const uint8_t *public_key,const uint8_t *message_hash,unsigned hash_size,const uint8_t *signature,uECC_Curve curve);
