...
static int json_token_streq(char *js, jsontok_t *t, char *s)
{
...
static int json_str_to_bool(json_parser_t *jobj, jsontok_t *t, int *value)
{
...
int json_get_val_bool(json_parser_t *jobj, char *key, int *value)
{
	jsontok_t *t;
...
int json_array_get_bool(json_parser_t *jobj, uint16_t index, int *value)
{
	jsontok_t *t;
...
int json_is_object(json_parser_t *jobj)
{
...
int json_is_array(json_parser_t *jobj)
{
	if (jobj->cur->type == JSMN_ARRAY)
		return true;
	else
