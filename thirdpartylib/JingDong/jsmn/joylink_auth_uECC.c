...
#define REPEAT_0(...)
#define REPEAT_SOME(N, stuff) DEFER(CONCAT(REPEAT_NAME_, SOME_OR_0(DEC1(N))))()(DEC1(N), stuff) stuff
#define REPEAT1(N, stuff) EVAL(REPEAT_SOME(N, stuff))

#define REPEATM_NAME_0() REPEATM_0
#define REPEATM_NAME_SOME() REPEATM_SOME
#define REPEATM_0(...)
#define REPEATM_SOME(N, macro) macro(N) \
    DEFER(CONCAT(REPEATM_NAME_, SOME_OR_0(DEC1(N))))()(DEC1(N), macro)
...
static uECC_RNG_Function_sap g_rng_function = &default_RNG;
#else
static uECC_RNG_Function_sap g_rng_function = 0;
#endif

void uECC_set_rng_sap(uECC_RNG_Function_sap rng_function) 
{
...
uECC_RNG_Function_sap uECC_get_rng_sap(void) 
{
...
int uECC_make_key_sap(uint8_t *public_key,
                  uint8_t *private_key,
                  uECC_Curve curve) {
 ...
 int uECC_shared_secret_sap(const uint8_t *public_key,
...
#if uECC_SUPPORT_COMPRESSED_POINT
 void uECC_compress_sap(const uint8_t *public_key, uint8_t *compressed, uECC_Curve curve) {
    wordcount_t i;
 ...
 void uECC_decompress_sap(const uint8_t *compressed, uint8_t *public_key, uECC_Curve curve) {
 ...
 int uECC_valid_public_key_sap(const uint8_t *public_key, uECC_Curve curve) {
    uECC_word_t public[uECC_MAX_WORDS * 2];
 ...
 int uECC_compute_public_key_sap(const uint8_t *private_key, uint8_t *public_key, uECC_Curve curve) {
    uECC_word_t private[uECC_MAX_WORDS];
    uECC_word_t public[uECC_MAX_WORDS * 2];
...
/* -------- ECDSA code -------- */
 ...
 int uECC_sign_sap(const uint8_t *private_key,
              const uint8_t *message_hash,
              unsigned hash_size,
              uint8_t *signature,
              uECC_Curve curve) {
...
#if 0
 static void HMAC_init(const uECC_HashContext_t *hash_context, const uint8_t *K) 
{
 ...
 static void HMAC_update(const uECC_HashContext_t *hash_context,
                        const uint8_t *message,
                        unsigned message_size) {
    hash_context->update_hash(hash_context, message, message_size);
}

static void HMAC_finish(const uECC_HashContext_t *hash_context,const uint8_t *K,uint8_t *result) 
{
    uint8_t *pad = hash_context->tmp + 2 * hash_context->result_size;
    unsigned i;
    for (i = 0; i < hash_context->result_size; ++i)
...
/* V = HMAC_K(V) */
 static void update_V(const uECC_HashContext_t *hash_context, uint8_t *K, uint8_t *V) {
...
/* Deterministic signing, similar to RFC 6979. Differences are:
 ...
 int uECC_sign_deterministic_sap(const uint8_t *private_key,
                            const uint8_t *message_hash,
                            unsigned hash_size,
                            const uECC_HashContext_t *hash_context,
                            uint8_t *signature,
                            uECC_Curve curve) {
 ...
 int uECC_verify_sap(const uint8_t *public_key,
                const uint8_t *message_hash,
                unsigned hash_size,
...
void uECC_vli_mod_sqrt(uECC_word_t *a, uECC_Curve curve) 
{
 ...
 int jl3_uECC_verify_256r1(const uint8_t *public_key, const uint8_t *message_hash,unsigned hash_size,const uint8_t *signature)
 {
	 uint8_t dc_pubkey[64];
	 uECC_decompress_sap(public_key, dc_pubkey, uECC_secp256r1());
	 return uECC_verify_sap(dc_pubkey,message_hash, hash_size,signature,uECC_secp256r1());
 }
 int jl3_uECC_sign(const uint8_t *private_key,const uint8_t *message_hash,unsigned hash_size,uint8_t *signature,uECC_Curve curve) 
 {
	 uECC_word_t k[uECC_MAX_WORDS];
	 uECC_word_t tries;
 ...
 int jl3_uECC_verify(const uint8_t *public_key,const uint8_t *message_hash,unsigned hash_size,const uint8_t *signature,uECC_Curve curve) 
 {
	 uECC_word_t u1[uECC_MAX_WORDS], u2[uECC_MAX_WORDS];
	 uECC_word_t z[uECC_MAX_WORDS];
	 uECC_word_t public[uECC_MAX_WORDS * 2];
 ...
 int jl3_uECC_shared_secret(const uint8_t *public_key,const uint8_t *private_key,uint8_t *secret,uECC_Curve curve)
 {
	 uECC_word_t public[uECC_MAX_WORDS * 2];
	 uECC_word_t private[uECC_MAX_WORDS];
	 uECC_word_t tmp[uECC_MAX_WORDS];
