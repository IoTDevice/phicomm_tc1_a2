...
#if (uECC_CURVE == uECC_secp160r12 || uECC_CURVE == uECC_secp224r12) && (uECC_WORD_SIZE == 8)
...
#if (!asm_mult || (uECC_SQUARE_FUNC && !asm_square) || uECC_CURVE == uECC_secp256k12)
...
#if (uECC_CURVE == uECC_secp160r12 || uECC_CURVE == uECC_secp256k12)
...
#elif uECC_CURVE == uECC_secp192r12
...
#elif uECC_CURVE == uECC_secp256k12
...
#elif uECC_CURVE == uECC_secp224r12
...
static void HMAC_init(uECC_HashContext_t *hash_context, const uint8_t *K) {
...
static void HMAC_update(uECC_HashContext_t *hash_context,
                        const uint8_t *message,
                        unsigned message_size) {
    hash_context->update_hash(hash_context, message, message_size);
}

static void HMAC_finish(uECC_HashContext_t *hash_context, const uint8_t *K, uint8_t *result) {
    uint8_t *pad = hash_context->tmp + 2 * hash_context->result_size;
    unsigned i;
    for (i = 0; i < hash_context->result_size; ++i)
        pad[i] = K[i] ^ 0x5c;
...
/* V = HMAC_K(V) */
static void update_V(uECC_HashContext_t *hash_context, uint8_t *K, uint8_t *V) {
...
int uECC_sign_deterministic(const uint8_t private_key[uECC_BYTES],
                            const uint8_t message_hash[uECC_BYTES],
                            uECC_HashContext_t *hash_context,
                            uint8_t signature[uECC_BYTES*2]) {
